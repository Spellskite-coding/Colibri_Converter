...
2. **Retirer la dépendance** : remplacer pdf2docx/PyMuPDF par
   ...
   de tableaux à réécrire entièrement. Le sens DOCX → PDF, lui, n'a plus
   besoin de cette bascule : le moteur de rendu intégré (ReportLab, BSD) et
   python-docx (MIT) sont déjà publiables sous MIT indépendamment de ce
   choix — seul le sens PDF → DOCX ancre encore le projet en AGPL.
...
| python-docx | MIT | oui |
| ReportLab | BSD-3-Clause | oui |
| Pillow | MIT-CMU (HPND) | oui |
| Liberation / Carlito / Caladea (polices) | SIL OFL 1.1 | oui (`vendor/fonts/`) |
